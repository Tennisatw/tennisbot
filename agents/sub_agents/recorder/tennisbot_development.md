...
## 2026.01.10（补记：关于 MVP 与架构预判的感想）
- 记录一些我的感想：如果先做一个最小 MVP，再往上堆很多复杂功能，后面会变得有点复杂、很难收口。
- 以后构思任何软件的结构，我想从一开始就想清楚：这个软件“最复杂的形状”大概会是什么样。
- 以 Tennis Bot 为例：从最开始就应该预判到会用到 Web UI（以及围绕 Web UI 的一整套结构/边界）。
